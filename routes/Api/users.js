...
var jwt = require('jsonwebtoken'),
    middlewareAuth = require('../Api/token_verify');
const mongoose = require('mongoose')
const config = require('../../config/config');
const user = require('../../model/usermodal.js');
const userSchema = require('../../model/schemas/usermodal');
...
router.get('/getclinics', function (req, res, ) {
   user.getclinics((err,response)=>{
   if(err){
   res.status(200).json({"status":400,"err_field":"something went to the worng","result":""})
   }else{
...
user.getbyid(req,(err,response)=>{
if(err){
res.status(200).json({"sttaus":400,"err_field":"something went to the worng","result":""})
}else{
res.status(200).json({"sttaus":200,"err_field":"","result":response})
}
})
        ...
        user.findUserByEmail(req.body.email, function (err, data) {
            if (err) {
                ...
                if (data.status == 200 && data.data.length > 0) {
                    ...
                    user.comparePassword(req.body.password, data.data[0].password, (err, ismatch) => {
                        ...
                        if (err) {
                            ...
                            res.status(200).json({ "status": 400, "message": "invalid username or password", "error_field": "passwod does not matched" })
                        }
                    })
                    ...
                    res.status(200).json({ "status": 400, "message": "invalid username or password" });
...
router.post("/logindoctor", function (req, res) {
    if (!req.body.email) {
        ...
        user.findUserByEmaildoctor(req.body.email, function (err, data) {
            if (err) {
                res.status(400).send(err)
            } else {
...
router.post("/loginadmin", function (req, res) {
    if (!req.body.email) {
        ...
        user.findUserByEmailadmin(req.body.email, function (err, data) {
            if (err) {
                res.status(400).send(err)
            } else {
...
router.post("/change_password", middlewareAuth, function (req, res) {
    ...
    if (!req.body.password) {
        ...
        userSchema.find({ "_id": req.body._id }, function (err, data) {
                ...
                if (data.length > 0) {
                    ...
                    user.comparePassword(req.body.password, data[0].password, (err, ismatch) => {
                        ...
                        if (err) {
                            ...
                            res.status(200).json({ "status": 400, "message": "nvalid username old password", "error_field": "passwod does not matched" })
                        }
                    })


                } else {
                    res.status(200).json({ "status": 400, "message": "nvalid username old password", "error_field": "password" })
...
router.post("/upload_images", middlewareAuth, function (req, res) {
    let image = "";
    let imagePath = "";
    var imagesarray = [];
    var update_data = {};
    ...
    if (!req.body.userId) {
        ...
        userSchema.update({ _id: req.body.userId }, { $pushAll: { userImage: imagesarray }, "description": req.body.description }, { upsert: true }, function (err) {
            if (err) {
                res.status(200).json({ "status": 400, "err_field": "", "message": err });
            } else {
                userSchema.findOne({ "_id": req.body.userId }, function (err, data) {
                    if (err) {
...
router.post("/edit_profile", middlewareAuth, function (req, res) {
    ...
    if (!req.body.userId) {
        ...
        var user_update_data = { "fullname": req.body.fullname, "dateofbirth": req.body.dateofbirth, "gender": req.body.gender,"phonenumber":req.body.phonenumber };
        ...
        userSchema.update({ "_id": req.body.userId }, { $set: user_update_data }, function (err, data) {
                ...
                if (data.status == 200 && data.data.length > 0) {
                    ...
                    user.hashpassword(password, (err, randompassword) => {
                        if (err) {
                            ...
                            userSchema.update({ '_id': data.data[0]._id, }, { $set: { password: randompassword.result } }, function (err, updateduser) {
...
router.post("/viewdetails", middlewareAuth, function (req, res) {
...
router.get("/view/:userId", function (req, res) {
    ...
    if (!req.params.userId) {
        ...
        userSchema.findOne({ "_id": req.params.userId }, function (err, data) {
            if (err) {
                res.status(200).json({ "status": 400, "message": "Something Went wrong please try again", "err": err });
